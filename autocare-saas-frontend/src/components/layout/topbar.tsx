...
export function Topbar({ email }: { email: string }): React.JSX.Element {
  ...
  return (
    <header className="flex h-16 items-center justify-between border-b bg-white px-6 dark:bg-slate-950">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          className="relative rounded-lg p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-900"
        ...
        <div className="hidden items-center gap-2 border-l pl-3 sm:flex">
          <span className="grid size-8 place-items-center rounded-full bg-blue-100 text-xs font-bold text-blue-700">
            {email.slice(0, 1).toUpperCase()}
          </span>
          <span className="max-w-40 truncate text-sm font-medium">{email}</span>
